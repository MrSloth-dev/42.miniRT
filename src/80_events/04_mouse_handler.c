...
int	mouse_handler(int mousecode, int x, int y, t_canvas *canvas)
{
	...
	if (mousecode == 4 || mousecode == 5)
		canvas->camera.transf = ft_matrix_mult(canvas->camera.transf, canvas->camera.inv_rotate);
	if (mousecode == 4)
	{
		canvas->camera.transf = ft_matrix_mult(canvas->camera.transf, ft_translation_matrix(0, 0, -1));
	}
	else if (mousecode == 5)
		canvas->camera.transf = ft_matrix_mult(canvas->camera.transf, ft_translation_matrix(0, 0, 1));
	else
	...
	if (camera_invert == 1)
	{
		canvas->camera.transf = ft_matrix_mult(canvas->camera.transf, canvas->camera.rotate);
