...
int	ft_create_ambient(t_canvas *canvas, char **split)
{
	...
	color = (t_color){ft_atod(color_split[0]), ft_atod(color_split[1]),ft_atod(color_split[2])};
...
int	ft_create_camera(t_canvas *canvas, char **split)
{
	...
	coord = (t_tuple){ft_atod(coord_split[0]), ft_atod(coord_split[1]),ft_atod(coord_split[2]), 1};
	if (ft_check_null_split(coord_split)
		&& ft_vector_check(coord))
	{
	...
	norm = (t_tuple){ft_atod(norm_split[0]), ft_atod(norm_split[1]),ft_atod(norm_split[2]), 0};
...
int	ft_create_light(t_canvas *canvas, char **split)
{
		...
		&& ft_rgb_check(color))
	{
		...
		return (ft_free_split(color_split),
			ft_printf(2, "Error, light format is wrong\n"), 0);
	canvas->light[i].bright = ft_atoi(split[1]);
	canvas->count.light++;
